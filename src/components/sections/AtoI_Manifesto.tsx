...
import { release, bandInfo, timeline } from "@/data/content";
...
export function AtoI_Manifesto() {
  ...
  return (
    <Section id="manifesto" variant="ink-deep" pad="xl" className="overflow-hidden">
      <Container>
        <Eyebrow number="I">Manifesto</Eyebrow>

        <div className="mt-[clamp(3rem,6vi,5rem)] grid gap-12 lg:grid-cols-[auto_minmax(0,1fr)] lg:gap-20">
          {/* Oversized 57 — bleeds left margin */}
          <div className="relative -ml-[clamp(1rem,6vw,4rem)] flex items-start">
            <NumberScrub value={years} label="anos no palco" />
          </div>

          <div className="flex flex-col gap-10">
            <h2
              className="reveal-lead font-display -tracking-[0.02em] text-balance"
          ...
          <ol className="reveal-mid flex flex-col gap-0">
            {timeline.map((era, i) => {
              ...
              return (
                <li
                  key={i}
                  className="group relative flex flex-col gap-3 border-t border-border py-6 last:border-b"
                  style={{ ["--i" as string]: i } as React.CSSProperties}
                >
                  ...
                  <p className="max-w-[44ch] text-[0.88rem] leading-[1.6] text-text-muted text-pretty">
                    {era.description}
                  </p>
                </li>
              );
