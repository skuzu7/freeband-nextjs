...
- `npm run smoke` — Puppeteer: every route at 1440 and 390, fails on console errors, horizontal overflow, a cropped photograph, two photographs overlapping, a lightbox picture that does not fit the room its bars leave it (checked at 320×480 and 1440×720), or the legacy token link not reaching `/orcamento`. Needs a server up; honors `BASE_URL`, `ORCAMENTO_TOKEN`, `PUPPETEER_BROWSER_URL` (attach to a running Chrome), `PUPPETEER_EXECUTABLE_PATH`
...
CI (`.github/workflows/ci.yml`) runs `npm audit`, tests, lint, typecheck, build and both smoke tests against `next start` on every PR and push to `main`; Vercel deploys automatically from `main`. Node 22.18+ (`engines` in `package.json`).
...
`src/proxy.ts` gates every `/orcamento/*` path: a valid session cookie passes (and is re-issued in its last day); `/orcamento/<token>` never has a page — a matching token (percent-decoded first) is exchanged for a cookie and redirected to `/orcamento`, an already-authenticated visitor is redirected there too, anything else goes to `/admin`. `src/app/orcamento/page.tsx` verifies the cookie again: the proxy is the first gate, not the only one. The cookie is `"<exp>.<HMAC-SHA256(exp)>"` signed with Web Crypto (`src/lib/session.ts`), so the same code runs in the Edge proxy and Node server actions; named `__Host-freeband_admin` in production (`sessionCookieName()`), `freeband_admin` elsewhere; 7 days; `secretsMatch` compares secrets in constant time. Wrong passwords and wrong tokens both go through `src/lib/rateLimit.ts` (`FailureLimiter`: per address plus a shared bucket, in memory, per instance on Vercel). Security headers, including a static CSP, live in `next.config.ts`. The contract is pinned by `src/lib/__tests__/session.test.ts`, `proxy.test.ts` and `rateLimit.test.ts`; `docs/THREAT_MODEL.md` describes the same system.
...
- **Copy** — `src/data/band.ts` (identity, line-up, release, timeline, artists, partners), `src/data/packages.ts`, `src/data/contact.ts`, and per-page strings in `src/data/copy/*.ts` (`home`, `palco`, `arquivo`, `historia`, `portfolio` incl. the PDF pages, `admin`, `orcamento` incl. the proposal document and the form's defaults, `site` for nav/footer/SEO). Never hardcode copy in components — `bandInfo.brandLine` is the "Internacional" above the wordmark, `bandLineup.total` the number on stage (`src/data/__tests__/band.test.ts` checks the fold, the release highlights and the portfolio cover print it). `releaseShort` in `band.ts` is the band's one-line description reused by the meta description, `/palco` and the release — edit it there only. Copy that counts from today's date is a function, never a module-level constant: `yearsActive()` in `band.ts`, `site.footer.decades()`, and `release.manifesto/sloganFootnote/highlights(years)`. Server components call them per render (`revalidate = 86400` on the `(site)` layout re-renders a static build daily, but would never re-evaluate a module), the PDF calls them when generated, and `Fold` receives the year as a prop rather than reading the browser's clock.
- **Media** — `src/data/media/paths.ts` registers every image path; `frames.ts` (17 stage frames in three acts, grouped into plates), `figurinos.ts`, `estrutura.ts`, `reels.ts`, `posters.ts` (`home: true` marks the four flyers printed on the home), `hero.ts`. `public/social/` holds marketing exports that nothing on the site references. Every photo declares `aspect` as the file's real `W/H`; `src/data/__tests__/media.test.ts` checks it against disk (±2%), enforces the ≥900px long-edge floor for gallery photos, whitelists `quality` values against `next.config.ts`, and requires a blur entry for everything rendered.

### Photographs are never cropped — or covered

Every photo goes through `components/media/Photo.tsx`, whose box takes the file's real aspect, so `object-cover` has nothing to crop. No photograph sits on top of another either: the crop audit compares each box to its own file and cannot see occlusion, so the smoke test also fails when two photograph boxes intersect. On the home's thematic blocks that is why the wardrobe shot is framed beside the caption rather than tucked over the stage photograph's corner. Rows of photos are **plates** (`PlateRow`, `plateLayout()`): equal height, column widths in `fr` proportional to each photo's ratio; portraits stay side by side on phones, anything else stacks. The PDFs follow the same rule with `PlateRowPdf`. The one exception is the fold's stage poster, a backdrop under a scrim, marked `data-backdrop` so the smoke test skips it. Small 600×400 scans and 400×300 flyers exist only for the PDF and the archive.
...
- `LedPanel` — canvas that rasterises text, the wordmark or an image into a dot matrix and lights it once in view (`src/lib/led/rasterize.ts` is the pure, tested part; `sources.ts` the browser adapters; `palette.ts` reads the colour tokens and builds the ramp with a hex fallback). Budget: ≤6 000 dots, one rAF loop only while lighting, then a static frame. Under reduced motion the final frame is drawn at once. `dimDots`/`field` off and `fadeWhenLit` on when it sits over a photograph (the fold). The panel fails open: no 2D context, no observer or an exception all end in `onLit`, and a watchdog reveals the content if frames never come — the heading or photo behind it is never lost. The production stylesheet ships the oklch tokens as hex plus `lab()` (Lightning CSS), so `readColorVar` resolves whatever the browser computed by painting it on a 1×1 canvas; `parseColor` reads hex, `oklch()` and `rgb()`.
- `LedNumber` — digits in a real 5×7 matrix (SVG); `ledDots()` is shared with `LedNumberPdf`.
- `DotGrid` (CSS field), `.dot-line` (one row of the panel, a CSS utility), `Label` (caps + tracking; the site has no mono face). `Ticker` is the marquee's still, wrapped form under reduced motion; it never animates.
