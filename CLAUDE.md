...
- **Copy** — `src/data/band.ts` (identity, line-up, release, timeline, artists, partners), `src/data/packages.ts`, `src/data/contact.ts`, and per-page strings in `src/data/copy/*.ts` (`home`, `palco`, `arquivo`, `historia`, `portfolio` incl. the PDF pages, `admin`, `orcamento` incl. the proposal document and the form's defaults, `site` for nav/footer/SEO). Never hardcode copy in components — `bandInfo.brandLine` is the "Internacional" above the wordmark, `bandLineup.total` the number on stage (`src/data/__tests__/band.test.ts` checks every headline prints it). `releaseShort` in `band.ts` is the band's one-line description reused by the meta description, `/palco` and the release — edit it there only. Copy that counts from today's date (`bandInfo.yearsActive`, the footer's decades) is rendered on the server only; the `(site)` layout sets `revalidate = 86400` so a static build refreshes it, and `Fold` receives the year as a prop rather than reading the browser's clock.
