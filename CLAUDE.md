...
- `LedPanel` — canvas that rasterises text, the wordmark or an image into a dot matrix and lights it once in view (`src/lib/led/rasterize.ts` is the pure, tested part; `sources.ts` the browser adapters; `palette.ts` reads the colour tokens and builds the ramp with a hex fallback). Budget: ≤6 000 dots, one rAF loop only while lighting, then a static frame. Under reduced motion the final frame is drawn at once. `dimDots`/`field` off and `fadeWhenLit` on when it sits over a photograph (the fold). The panel fails open: no 2D context, no observer, an unparseable token or an exception all end in `onLit`, and a watchdog reveals the content if frames never come — the heading or photo behind it is never lost. The tokens arrive as `oklch()` strings; `mix()` parses those and hex only, never `rgb()`.
